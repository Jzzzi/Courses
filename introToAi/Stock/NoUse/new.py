...
def dataProcess(original_data, days_before=15, days_after=5, batch_size=400, k=0.8):
    """数据处理函数
    ...
    # 样本总数
    num_samples = len(original_data) - days_before - days_after - 10
...
# 预测上涨买入，预测下跌卖出，输出收益
    ...
    soldPoints = []
    for i in range(len(predicted)):
        if y_pred[i][1] > 0.98 and bought == False:
            print(f"上涨概率: {y_pred[i][1]}，下跌概率: {y_pred[i][0]}")
            bought = True
            bought_price = original_data.loc[i+15, 'close']
            boughtPoints.append(i)
        elif y_pred[i][0] > 0.5 and bought == True:
            print(f"上涨概率: {y_pred[i][1]}，下跌概率: {y_pred[i][0]}")
            bought = False
            money = money * (original_data.loc[i+15, 'close'] / bought_price - 0.001)
            total += 1
            soldPoints.append(i)
            if original_data.loc[i+15, 'close'] / bought_price > 1:
                correct += 1
    print(f'交易胜率: {correct/total}')
    print(f'收益: {money}')
    print(f'交易次数: {total}')
    # 保存模型
...
# 画图画出测试集的走势
    ...
    price = original_data.loc[15+int(num_samples*k):,'close'].values
    print(price.shape)
    plt.plot(price)
    for i in boughtPoints:
